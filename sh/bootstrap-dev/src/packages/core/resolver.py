...
class Resolver:
    ...
    def resolve(
        ...
        profile_components = self._get_profile_components(manifest, profile)
    ...
    def _get_profile_components(self, manifest: Dict, profile: str) -> List[str]:
        """Get component list for a profile."""
        if "profiles" not in manifest:
            raise ValidationError(
                "No profiles defined in manifest",
                "unknown",
