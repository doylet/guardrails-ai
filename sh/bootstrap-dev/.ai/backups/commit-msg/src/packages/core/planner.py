...
class Planner:
    ...
    def _discover_component_files(
        ...
        if "_plugin_path" in component_config:
            base_dir = Path(component_config["_plugin_path"])
        else:
    ...
    def _plan_file_action(
        ...
        # Determine component source type
        if "_plugin_path" in component_config:
            # Extract plugin name from plugin path
            plugin_path = Path(component_config["_plugin_path"])
            component_source = plugin_path.name
        else:
